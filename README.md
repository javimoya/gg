...
- **dev ≠ launched.** No defensive data engineering while the product is still in development; in `dev`, recreate and reseed freely. But your **own** on-disk data is still protected in both stages: recreating a populated store always stops for a yes.
