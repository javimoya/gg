---
description: "Opens the next batch in ONE ceremony — capture, triage, and design together, one gate. Bring whatever you have (a pasted list of bugs/ideas, items from the backlog, one thing or ten): it records each with a stable B-NN, folds duplicates, weighs every item S/M/L, grills ONLY what has design weight (a decided bug gets zero questions), and writes the board /gg:go builds from. Exactly one consolidated veto gate, always. Also the lane for a research batch (an open question closed by a measured answer) and for re-scoping a batch already under way when a show changed the plan."
model: inherit
...
- **`state: building`** → a batch is under way. **Default: stop** and route to `/gg:go` — never
  silently clobber a live board. The one exception is a **re-scope in place**: a show's look (or the
...
**First, set `state: shaping` in WORK** — the whole arc runs under it (any death, graceful or not,
resumes via §0's shaping path; §4 flips it to `building`). Then gather the batch's candidate set:
...
- **What the user points at** in `BACKLOG.md ## New` / `## Later` (by id, or "and the backlog ones").
  Items *not* brought in stay where they are — never invisible: open with one line of counts
  (*"backlog holds {K} new · {J} later"*) when anything stays behind.
...
`## Last closes` intact**. The Board: one row per item, sized so each fits one fresh session (an L
item may span several rows; S items are one row each), each row's **done when** (a `[bug]` row's
carries the broken-vs-expected essence), the **show** rows only where a `[discovered]` clause in
play becomes judgeable (riskiest first, as early as the foundation allows — a bug-batch needs no
show), and the `## Try` block (deliverable + how to see it + the load-bearing flows, citing B-NNs —
